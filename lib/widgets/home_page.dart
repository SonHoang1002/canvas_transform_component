...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final transformationController = context.canvasTransformCubitSelect(
      (value) => value.state,
    );
    final tool = context.toolCubitSelect((value) => value.state);
    final isToolHand = context.toolCubitSelect(
      (value) => value.state == ToolType.hand,
    );
    //
    ...
    return RawKeyboardListener(
      ...
      onKey: (event) {
        ...
        final shortcuts = {
          const SingleActivator(LogicalKeyboardKey.keyV): VoidCallbackIntent(
            context.read<ToolCubit>().setMove,
          ),
          const SingleActivator(LogicalKeyboardKey.keyF): VoidCallbackIntent(
            context.read<ToolCubit>().setFrame,
          ),
          const SingleActivator(LogicalKeyboardKey.keyR): VoidCallbackIntent(
          ...
          const SingleActivator(LogicalKeyboardKey.keyT): VoidCallbackIntent(
              ...
              child: Stack(
                children: [
                  ...
                  ValueListenableBuilder(
                    valueListenable: transformationController,
                    builder:
                        (context2, transform, child) => Stack(
                          clipBehavior: Clip.none,
                          children: [
                            // unselect all
                            GestureDetector(
                          ...
                          onInteractionUpdate: (details) {
                            if (middleClick ||
                                (isToolHand &&
                                    (!isNotCanvasTooling || isCanvasTooling)) ||
                                (isCanvasTooling && !leftClick && !isZooming)) {
                              context.read<CanvasTransformCubit>().update(
